...
export function annotateGraph(graph, request) {
    ...
    const dustThreshold = Math.max(ANALYSIS_THRESHOLDS.dustAbsoluteAmount, Number((seedOutgoingTotal * ANALYSIS_THRESHOLDS.dustShareOfSeed).toFixed(6)));
...
export function applyRiskAdjustments(graph, scored) {
...
export function propagateValueFlow(graph, seedNodeIds) {
    const maps = buildGraphMaps(graph);
    const sortedEdges = [...graph.edges].sort((left, right) => left.timestamp.localeCompare(right.timestamp));
    const remainingFlowByNode = new Map();
    const receivedFlowByNode = new Map();
    const remainingAmountByEdge = new Map();
...
export function extractSuspiciousPaths(graph, findings, seedNodeIds, maxPaths) {
    const maps = buildGraphMaps(graph);
...
export function pruneGraph(graph, suspiciousPaths, seedNodeIds, maxNodes) {
    if (graph.nodes.length <= maxNodes) {
        return { graph };
    }
    const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
    const rankedNodes = [...graph.nodes].sort((left, right) => rankNodeForRetention(right) - rankNodeForRetention(left));
    const keepNodeIds = new Set(seedNodeIds);
    ...
    keptEdges = keptEdges.filter((edge) => edge.flags.includes("path-highlight") ||
        edge.riskScore >= 20 ||
...
export function sliceGraph(graph, options) {
    const maps = buildGraphMaps(graph);
    ...
    if (options.focusNodeId && maps.nodeById.has(options.focusNodeId)) {
        const queue = [{ nodeId: options.focusNodeId, depth: 0 }];
...
export function filterFindingsForGraph(findings, graph) {
    const nodeIds = new Set(graph.nodes.map((node) => node.id));
    const edgeIds = new Set(graph.edges.map((edge) => edge.id));
    return findings.filter((finding) => finding.relatedNodeIds.some((nodeId) => nodeIds.has(nodeId)) ||
        finding.relatedEdgeIds.some((edgeId) => edgeIds.has(edgeId)));
}
function buildGraphMaps(graph) {
...
function getSeedNodeIds(graph, request) {
...
function getSeedOutgoingTotal(graph, seedNodeIds) {
    return graph.edges
        .filter((edge) => seedNodeIds.includes(edge.from))
        .reduce((sum, edge) => sum + edge.amount, 0);
}
function labelBridgeTouchpoints(graph, maps) {
    for (const edge of graph.edges) {
...
function labelBehavioralRouters(graph, maps) {
    for (const node of graph.nodes) {
        ...
        if (uniqueSenders.size >= ANALYSIS_THRESHOLDS.routerCounterparties &&
...
function labelBehavioralExchanges(graph, maps, dustThreshold) {
    for (const node of graph.nodes) {
...
function labelMultisigCandidates(graph, maps) {
    for (const node of graph.nodes) {
...
function applyDustFilter(graph, dustThreshold) {
    for (const edge of graph.edges) {
...
function applyAirdropFilter(graph, maps, amountThreshold) {
    for (const node of graph.nodes) {
...
function walkPaths(graph, maps, findings, seedNodeId, currentNodeId, nodeIds, edgeIds, visited, paths, seen) {
    if (edgeIds.length >= 6) {
    ...
    for (const edge of outgoing) {
        ...
        walkPaths(graph, maps, findings, seedNodeId, edge.to, [...nodeIds, edge.to], [...edgeIds, edge.id], nextVisited, paths, seen);
    }
    maybePushPath(graph, findings, seedNodeId, nodeIds, edgeIds, paths, seen);
}
function maybePushPath(graph, findings, seedNodeId, nodeIds, edgeIds, paths, seen) {
    if (edgeIds.length === 0) {
        return;
    }
    const edges = edgeIds
    ...
    const riskScore = Math.round(edges.reduce((sum, edge) => sum + edge.riskScore, 0) +
        nodes.reduce((sum, node) => sum + node.riskScore * 0.2, 0));
    const confidence = roundMetric(combineConfidence([
...
function deriveDominantReason(edges, nodes, findings) {
...
function shouldAlwaysRetainNode(node) {
    return (node.riskScore >= 40 ||
...
function rankNodeForRetention(node) {
    const kindBonus = node.kind === "mixer" || node.kind === "bridge"
        ? 30
