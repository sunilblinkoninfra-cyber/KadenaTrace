    ...
    describe("scoreGraph", () => {
        ...
        it("should detect fan-out burst pattern", () => {
            ...
            const nodes = [
                ...
                ...Array.from({ length: 6 }, (_, i) => createMockNode({ id: `recipient-${i}`, address: `0xrecipient${i}` }))
            ];
            const edges = [
                // Fan out from source to 6 recipients within 5 minutes
                ...Array.from({ length: 6 }, (_, i) => createMockEdge({
                    id: `edge-${i}`,
                    from: "source",
                    to: `recipient-${i}`,
        ...
        it("should detect sink consolidation", () => {
            const nodes = [
                ...Array.from({ length: 5 }, (_, i) => createMockNode({ id: `source-${i}`, address: `0xsource${i}` })),
                createMockNode({
                    id: "sink",
                    address: "0xsink",
                    kind: "exchange",
        ...
        it("should not flag dust transactions", () => {
            ...
            assert.strictEqual(result.findings.filter((f) => f.code === "fan-out-burst").length, 0, "Should not flag dust transactions as fan-out");
        });
