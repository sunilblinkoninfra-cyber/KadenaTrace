...
function upsertNodeAdjustment(map, nodeId, signal) {
...
function upsertEdgeAdjustment(map, edgeId, signal, flag) {
...
function applyFanOut(graph, accumulator) {
    ...
    for (const [nodeId, edges] of outgoing) {
        ...
        if (uniqueRecipients.size >= HEURISTIC_THRESHOLDS.fanOutRecipients &&
            deltaMinutes <= HEURISTIC_THRESHOLDS.fanOutWindowMinutes) {
            const signal = createSignal("fan-out-burst", "Fan-out burst", `Fan-out burst to ${uniqueRecipients.size} recipients within ${Math.round(deltaMinutes)} minutes.`, 32, 0.86);
...
function applyLargeSplit(graph, accumulator) {
    const outgoing = new Map();
    for (const edge of graph.edges.filter((e) => !isSuppressedEdge(e))) {
        const list = outgoing.get(edge.from) ?? [];
        list.push(edge);
        outgoing.set(edge.from, list);
    }
    for (const [nodeId, edges] of outgoing) {
        if (edges.length < HEURISTIC_THRESHOLDS.largeSplitMinOutputs) {
            continue;
        }
        const sorted = [...edges].sort((a, b) => a.timestamp.localeCompare(b.timestamp));
        const first = sorted[0];
        const last = sorted[sorted.length - 1];
        const windowMinutes = (Date.parse(last.timestamp) -
        ...
        const significantSplits = edges.filter((e) => e.amount / totalAmount >=
            HEURISTIC_THRESHOLDS.largeSplitMinSplitRatio);
        if (significantSplits.length <
            HEURISTIC_THRESHOLDS.largeSplitMinOutputs) {
            continue;
        }
        const signal = createSignal("large-split", "Large-value split", `${significantSplits.length} large-value outputs within ` +
            ...
            `thresholds.`, 20, 0.82);
        accumulator.findings.push(createFinding(signal, "high", [nodeId, ...significantSplits.map((e) => e.to)], significantSplits));
        upsertNodeAdjustment(accumulator.nodeAdjustments, nodeId, signal);
        for (const edge of significantSplits) {
            upsertEdgeAdjustment(accumulator.edgeAdjustments, edge.id, signal, "large-split");
        }
    }
}
function applyFanIn(graph, accumulator) {
    ...
    for (const [nodeId, edges] of inbound) {
        ...
        const signal = createSignal("fan-in-consolidation", "Fan-in consolidation", `A non-exchange destination consolidated funds from ${uniqueSenders.size} senders.`, 20, 0.72);
...
function applyRapidHops(graph, accumulator) {
    ...
    for (const startEdge of candidateEdges) {
        ...
        const visitedNodeIds = new Set([startEdge.from, startEdge.to]);
        ...
        const signal = createSignal("rapid-hop-path", "Rapid hop path", `Rapid laundering path spans ${path.length} hops with less than ` +
            `${HEURISTIC_THRESHOLDS.rapidHopWindowMinutes} minutes between each hop.`, 18, 0.91);
        accumulator.findings.push(createFinding(signal, "critical", Array.from(new Set(path.flatMap((item) => [item.from, item.to]))), path));
        for (const item of path) {
...
function applyBridgeUsage(graph, accumulator) {
    for (const edge of graph.edges) {
        if (!edge.bridgeTransferId && !edge.synthetic) {
            continue;
        }
        const isKnownBridge = edge.metadata?.trustedBridge === true;
        const signal = createSignal("bridge-usage", "Bridge usage", isKnownBridge
            ? "Funds crossed a labeled bridge route. This is context, not a standalone fraud signal."
            : "Funds crossed into or out of a supported bridge route.", isKnownBridge ? 4 : 8, isKnownBridge ? 0.35 : 0.5);
...
function applyBridgeBurst(graph, accumulator) {
    const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
    const bridgeEventMap = new Map();
    for (const edge of graph.edges.filter((candidate) => !isSuppressedEdge(candidate))) {
        const fromNode = nodeById.get(edge.from);
        const toNode = nodeById.get(edge.to);
        const isBridgeEdge = Boolean(edge.bridgeTransferId) ||
    ...
    for (let index = 0; index < bridgeEvents.length; index += 1) {
        ...
        for (let cursor = index + 1; cursor < bridgeEvents.length; cursor += 1) {
            ...
            const minutes = (Date.parse(candidate.timestamp) - Date.parse(startEvent.timestamp)) / 1000 / 60;
        ...
        const relatedEdges = Array.from(new Map(clusteredEvents
            .flatMap((event) => event.edges)
            .map((edge) => [edge.id, edge])).values());
        const relatedNodeIds = Array.from(new Set(relatedEdges.flatMap((edge) => [edge.from, edge.to])));
        const uniqueChains = new Set(relatedEdges.map((edge) => edge.chain));
        const signal = createSignal("bridge-burst", "Bridge burst", `Detected ${clusteredEvents.length} bridge-driven transfers across ${uniqueChains.size} chain windows ` +
            `within ${HEURISTIC_THRESHOLDS.bridgeBurstWindowMinutes} minutes — a rapid cross-chain fragmentation pattern ` +
            `used to break the audit trail.`, 26, 0.84);
        accumulator.findings.push(createFinding(signal, "high", relatedNodeIds, relatedEdges));
...
function applyBridgeObfuscation(graph, accumulator) {
    const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
    for (const edge of graph.edges.filter((item) => item.bridgeTransferId || item.flags.includes("bridge"))) {
        ...
        const signal = createSignal("bridge-obfuscation", "Bridge obfuscation", `Funds bridged and then reached a ${destinationNode.kind} within ${HEURISTIC_THRESHOLDS.bridgeObfuscationWindowMinutes} minutes.`, 24, 0.82);
        accumulator.findings.push(createFinding(signal, "high", [edge.from, edge.to, continuation.to], [edge, continuation]));
...
function applyMixerInteraction(graph, accumulator) {
    for (const node of graph.nodes) {
        ...
        const signal = createSignal("mixer-touchpoint", "Mixer touchpoint", "Funds interacted with a wallet or contract labeled as a mixer.", 40, 0.96);
...
function applySinkConsolidation(graph, accumulator) {
    ...
    for (const node of graph.nodes) {
        ...
        const hasRiskyPrecursor = supportingEdges.some((edge) => edge.flags.includes("rapid-hop") ||
        ...
        if (ratio >= HEURISTIC_THRESHOLDS.sinkConsolidationRatio && node.tags.includes("sink") && hasRiskyPrecursor) {
            const signal = createSignal("sink-consolidation", "Sink consolidation", node.kind === "exchange"
                ? `A cashout path consolidated ${(ratio * 100).toFixed(1)}% of observed value into an exchange destination.`
                : `A sink wallet absorbed ${(ratio * 100).toFixed(1)}% of observed value.`, 22, node.kind === "exchange" ? 0.74 : 0.8);
            accumulator.findings.push(createFinding(signal, "high", [node.id, ...supportingEdges.map((edge) => edge.from)], supportingEdges));
            upsertNodeAdjustment(accumulator.nodeAdjustments, node.id, signal);
            for (const edge of supportingEdges) {
                upsertEdgeAdjustment(accumulator.edgeAdjustments, edge.id, { ...signal, weight: 12 }, node.kind === "exchange" ? "exchange-cashout" : "sink");
            }
        }
    }
}
function applyPeelChain(graph, accumulator) {
    ...
    for (const edge of candidateEdges) {
        ...
        const chain = [edge];
        const visitedNodeIds = new Set([edge.from, edge.to]);
        let cursor = edge.to;
        for (let depth = 0; depth < 10; depth++) {
            ...
            const dominant = outbound.find((candidate) => {
                ...
                return (retained <= HEURISTIC_THRESHOLDS.peelChainMaxRetainPct &&
        ...
        const signal = createSignal("peel-chain", "Peel chain", `Peel chain of ${chain.length} hops: each hop forwards >85% of funds, ` +
            `retaining a small amount to obscure the total. Classic layering technique.`, 26, 0.84);
        accumulator.findings.push(createFinding(signal, "high", Array.from(new Set(chain.flatMap((item) => [item.from, item.to]))), chain));
...
function applyStructuring(graph, accumulator) {
    const outgoing = new Map();
    for (const edge of graph.edges.filter((candidate) => !isSuppressedEdge(candidate))) {
        const list = outgoing.get(edge.from) ?? [];
        list.push(edge);
    ...
    for (const [nodeId, edges] of outgoing) {
        ...
        const roundEdges = edges.filter((edge) => {
            ...
            return (remainder <= HEURISTIC_THRESHOLDS.structuringRoundAmountTolerance ||
        ...
        const signal = createSignal("structuring", "Structuring / smurfing", `${roundEdges.length} of ${edges.length} outgoing transactions use ` +
            `suspiciously round amounts — a classic structuring pattern used to ` +
            `evade exchange AML reporting thresholds.`, 22, 0.78);
        accumulator.findings.push(createFinding(signal, "high", [nodeId, ...roundEdges.map((edge) => edge.to)], roundEdges));
        upsertNodeAdjustment(accumulator.nodeAdjustments, nodeId, signal);
...
function applyCircularFlow(graph, accumulator) {
    const candidateEdges = graph.edges.filter((candidate) => !isSuppressedEdge(candidate));
    for (const edge of candidateEdges) {
        ...
        const returning = candidateEdges.find((candidate) => {
            ...
            const minutesLater = (Date.parse(candidate.timestamp) - Date.parse(edge.timestamp)) / 1000 / 60;
            return minutesLater > 0 && minutesLater <= HEURISTIC_THRESHOLDS.circularFlowMaxMinutes;
        });
        if (!returning) {
            continue;
        }
        const signal = createSignal("circular-flow", "Circular flow", `Funds sent from ${edge.from.split(":")[1]?.slice(0, 10) ?? edge.from} ` +
            `returned to the same wallet within ${HEURISTIC_THRESHOLDS.circularFlowMaxMinutes} minutes. ` +
            `Indicates wash trading or self-dealing to create artificial transaction history.`, 30, 0.88);
        accumulator.findings.push(createFinding(signal, "high", [edge.from, edge.to, returning.from], [edge, returning]));
        upsertNodeAdjustment(accumulator.nodeAdjustments, edge.from, signal);
...
function applyDormantReactivation(graph, accumulator) {
    ...
    for (const nodeId of inboundByNode.keys()) {
        ...
        const dormantDays = (Date.parse(firstSentAfter.timestamp) - Date.parse(lastReceived)) /
            1000 /
            60 /
            60 /
            24;
        ...
        const signal = createSignal("dormant-reactivation", "Dormant wallet reactivation", `Wallet was dormant for ${Math.round(dormantDays)} days then suddenly ` +
            `forwarded funds — a sleeper wallet pattern common in staged long-term laundering.`, 28, 0.81);
        accumulator.findings.push(createFinding(signal, "high", [nodeId, firstSentAfter.to], [firstSentAfter]));
        upsertNodeAdjustment(accumulator.nodeAdjustments, nodeId, signal);
        upsertEdgeAdjustment(accumulator.edgeAdjustments, firstSentAfter.id, signal, "dormant-reactivation");
    }
}
function applyExchangeHopping(graph, accumulator) {
    const nodeById = new Map(graph.nodes.map((node) => [node.id, node]));
    const outboundByNode = new Map();
    for (const edge of graph.edges.filter((candidate) => !isSuppressedEdge(candidate))) {
        const list = outboundByNode.get(edge.from) ?? [];
    ...
    for (const [nodeId, edges] of outboundByNode) {
        ...
        const sorted = [...exchangeEdges].sort((left, right) => left.timestamp.localeCompare(right.timestamp));
        const first = sorted[0];
        const last = sorted[sorted.length - 1];
        if (!first || !last) {
            continue;
        }
        const windowMinutes = (Date.parse(last.timestamp) - Date.parse(first.timestamp)) / 1000 / 60;
        ...
        const signal = createSignal("exchange-hopping", "Exchange hopping", `Funds deposited into ${uniqueExchanges.size} different exchanges within ` +
            `${Math.round(windowMinutes)} minutes — fragmenting the KYC trail across ` +
            `exchange reporting silos to obscure the total amount moved.`, 24, 0.83);
        accumulator.findings.push(createFinding(signal, "high", [nodeId, ...Array.from(uniqueExchanges)], exchangeEdges));
        upsertNodeAdjustment(accumulator.nodeAdjustments, nodeId, signal);
...
function createSignal(code, title, reason, weight, confidence) {
    return {
        code,
        title,
        reason,
...
function createFinding(signal, severity, relatedNodeIds, edges) {
    return {
        code: signal.code,
        severity,
...
function isSuppressedEdge(edge) {
