// explainable-risk.ts -- Converts heuristic signals into investigator-facing, reproducible risk explanations.
import type {
  ExplainablePathRiskItem,
  ExplainableRiskItem,
  GraphNode,
  RiskAnalysis,
  SuspiciousPath,
...
export function buildRiskAnalysis(graph: TraceGraph, suspiciousPaths: SuspiciousPath[]): RiskAnalysis {
...
function toExplainableWallet(node: GraphNode): ExplainableRiskItem {
...
function toExplainablePath(path: SuspiciousPath): ExplainablePathRiskItem {
...
function combineExplanation(primaryReason: string, signals: string[]): string {
  if (signals.length === 0) {
...
function extractSignalsFromReason(reason: string): string[] {
  const dictionary = [
    "fan-out",
    "rapid hop",
    "bridge usage",
...
function toInvestigatorRisk(score: number): "HIGH" | "MEDIUM" | "LOW" {
  if (score >= 60) {
    return "HIGH";
  }
  if (score >= 30) {
