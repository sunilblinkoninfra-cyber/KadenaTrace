...
import type { Finding, GraphEdge, GraphNode, TraceGraph } from "../domain.js";
import {
  annotateGraph,
      ...
      const graph: TraceGraph = {
        nodes: [
          ...
          ...Array.from({ length: 10 }, (_, i) =>
            createMockNode({ id: `node-${i}`, riskScore: 70 })
          )
        ],
        edges: Array.from({ length: 10 }, (_, i) =>
          createMockEdge({ id: `e-${i}`, from: "seed", to: `node-${i}` })
        )
      };

      const findings: Finding[] = graph.nodes.slice(1).map((node, i) => ({
        code: "high-risk",
        severity: "high",
    ...
    it("should respect limit parameter", () => {
      const graph: TraceGraph = {
        ...
        edges: Array.from({ length: 19 }, (_, i) =>
          createMockEdge({ id: `e-${i}`, from: `n-${i}`, to: `n-${i + 1}` })
        )
      };
